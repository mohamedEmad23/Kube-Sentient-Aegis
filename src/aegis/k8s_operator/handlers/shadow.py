...
from aegis.shadow.errors import ensure_shadow_error
    ...
    while retry_ctx.attempt <= retry_ctx.max_retries:
        ...
        try:
            ...
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_verification_error",
                phase="operator_shadow_verification",
                details={"deployment": deployment_name, "namespace": namespace},
            )
            last_error = shadow_error.message
            log.exception(
                "shadow_verification_error",
                deployment=deployment_name,
                attempt=retry_ctx.attempt,
                error=shadow_error.to_dict(),
            )
