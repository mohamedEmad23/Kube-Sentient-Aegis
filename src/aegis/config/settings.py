...
class KubernetesSettings(BaseSettings):
    ...
    @classmethod
    def normalize_kubeconfig_path(cls, value: str | None) -> str | None:
        """Normalize kubeconfig path values from env/.env.

        Handles common `.env` forms such as:
        - `$HOME/.kube/config`
        - `~/.kube/config`
        - empty strings
        """
    ...
    def normalize_context(cls, value: str | None) -> str | None:
        """Treat blank context values as unset."""
        if value is None:
            return None
        cleaned = value.strip()
