...
def _fetch_k8s_context(
    ...
    try:
        if settings.kubernetes.in_cluster:
            ...
            context_candidates = [settings.kubernetes.context] if settings.kubernetes.context else [None]
            if settings.kubernetes.context:
                # Fall back to active kubeconfig context when configured context is stale.
            ...
            for context_candidate in context_candidates:
                ...
                if last_error:
                    raise last_error
                raise k8s_config.ConfigException("Unable to load Kubernetes kubeconfig")
    except k8s_config.ConfigException as exc:
