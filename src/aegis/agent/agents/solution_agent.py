...
from aegis.agent.llm.huggingface import get_huggingface_client
...
async def solution_agent(
    ...
    # Using Hugging Face Inference API for solution generation
    ...
    try:
        ...
        # Using default Hugging Face model (mistralai/Mistral-7B-Instruct-v0.3)
        fix_proposal: FixProposal = hf_client.chat_with_schema(
            messages=messages,
            schema=FixProposal,
            model=None,  # Use HF client default instead of settings (which is for Ollama)
