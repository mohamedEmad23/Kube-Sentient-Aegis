...
def _as_shadow_error(
    ...
    *,
    ...
    """Convert arbitrary error payloads into a structured shadow error."""
    if isinstance(error, Exception):
        return ensure_shadow_error(
            error,
            code=code,
            phase=phase,
            retryable=retryable,
    ...
    parsed = parse_shadow_error(error)
    if parsed:
        return parsed
...
def _print_shadow_error_panel(
    ...
    *,
    code: str,
    phase: str,
    title: str = "Shadow Error",
    retryable: bool = False,
    details: dict[str, Any] | None = None,
) -> None:
    """Render a user-facing, structured shadow failure panel."""
    shadow_error = _as_shadow_error(
        error,
        code=code,
        phase=phase,
        retryable=retryable,
    ...
    details_block = ""
    if shadow_error.details:
        details_json = json.dumps(shadow_error.details, indent=2, sort_keys=True, ensure_ascii=True)
        details_block = f"\n\n[bold]Details:[/bold]\n{details_json}"

    console.print(
        Panel(
            f"[bold]Code:[/bold] {shadow_error.code}\n"
            f"[bold]Phase:[/bold] {shadow_error.phase}\n"
            f"[bold]Message:[/bold] {shadow_error.message}\n"
            f"[bold]Retryable:[/bold] {shadow_error.retryable}{details_block}",
...
def _shadow_manager_init_error(error: Exception) -> ShadowWorkflowError:
    """Normalize shadow manager initialization failures."""
    code = "kubeconfig_invalid" if isinstance(error, k8s_config.ConfigException) else "shadow_init_failed"
    return ensure_shadow_error(
        error,
        code=code,
        phase="shadow_manager_init",
        retryable=False,
        details={
...
def _get_shadow_manager_for_cli(
    console: Console,
    *,
    fatal: bool = True,
) -> tuple[Any | None, dict[str, Any] | None]:
    """Get shadow manager with structured, user-facing initialization errors."""
    try:
        return get_shadow_manager(), None
    except Exception as exc:
        shadow_error = _shadow_manager_init_error(exc)
        log.error("shadow_manager_init_failed", error=shadow_error.to_dict())
        _print_shadow_error_panel(
            console,
            shadow_error,
            code=shadow_error.code,
            phase=shadow_error.phase,
            title="Shadow Initialization Failed",
        )
        if fatal:
            raise typer.Exit(code=1) from None
        return None, shadow_error.to_dict()
...
def _run_shadow_verification(
    *,
    ...
    """Execute shadow verification and return result payload."""
    shadow_manager, init_error = _get_shadow_manager_for_cli(console, fatal=False)
    if shadow_manager is None:
        return None, False, None, None, init_error
        ...
        try:
            ...
            if env:
                if env.logs:
                    logs = "\n".join(env.logs)
                if env.error:
                    parsed_error = parse_shadow_error(env.error)
                    if parsed_error:
                        error_result = parsed_error.to_dict()
                        error_line = (
                            f"ERROR [{parsed_error.code}] ({parsed_error.phase}) "
                            f"{parsed_error.message}"
                        )
                        logs = f"{logs}\n{error_line}" if logs else error_line
    ...
    with console.status("[bold blue]Running shadow verification..."):
        try:
            shadow_id, passed, logs, security_results, error_result = asyncio.run(_execute())
        except Exception as e:  # pragma: no cover - runtime path
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_verification_failed_cli",
...
def _maybe_run_shadow_verification(
    *,
    ...
    if (
        ...
        shadow_id, passed, logs, security_results, shadow_error = _run_shadow_verification(
            console=console,
            resource_type=resource_type,
            resource_name=resource_name,
            namespace=namespace,
        ...
        result["shadow_error"] = shadow_error
        ...
        if passed is None:
            ...
            if shadow_error:
                details += (
                    ...
                    f"  • code: {shadow_error.get('code')}\n"
                    f"  • phase: {shadow_error.get('phase')}\n"
                    f"  • message: {shadow_error.get('message')}\n"
                    f"  • retryable: {shadow_error.get('retryable')}"
                )
...
def _generate_abort_report(
    *,
    ...
    Args:
        ...
        shadow_error: Structured shadow error payload
    ...
    *,
    ...
    if apply_fix:
        ...
        report = _generate_abort_report(
            ...
            shadow_error=result.get("shadow_error"),
        )
...
def analyze(
    ...
    # Mock mode should not require live LLM provider availability.
    if mock:
        log.info("llm_availability_check_skipped", reason="mock_mode")
    else:
...
def shadow_create(
    ...
    shadow_manager, _ = _get_shadow_manager_for_cli(console)
    if shadow_manager is None:
        raise typer.Exit(code=1)
    ...
    if wait:
        with console.status(f"[bold green]Creating shadow environment for {source_resource}..."):
            try:
                ...
                shadow_error = ensure_shadow_error(
                    e,
                    code="shadow_create_failed_cli",
                    phase="cli_shadow_create",
                    details={"source_resource": source_resource, "namespace": namespace},
                )
                log.error("shadow_create_failed", error=shadow_error.to_dict())
                _print_shadow_error_panel(
                    console,
                    shadow_error,
                    code="shadow_create_failed_cli",
                    phase="cli_shadow_create",
                    title="Shadow Creation Failed",
                )
                raise typer.Exit(code=1) from None
    else:
        ...
        try:
            ...
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_create_background_failed",
                phase="cli_shadow_create",
                details={"source_resource": source_resource, "namespace": namespace},
            )
            log.error("shadow_create_failed", error=shadow_error.to_dict())
            _print_shadow_error_panel(
                ...
                code="shadow_create_background_failed",
                phase="cli_shadow_create",
                title="Shadow Background Creation Failed",
            )
            raise typer.Exit(code=1) from None
...
def shadow_list() -> None:
    ...
    try:
        ...
        shadow_error = ensure_shadow_error(
            exc,
            code="shadow_list_failed_cli",
            phase="cli_shadow_list",
        )
        log.error("shadow_list_failed", error=shadow_error.to_dict())
        _print_shadow_error_panel(
            console,
            shadow_error,
            code="shadow_list_failed_cli",
            phase="cli_shadow_list",
            title="Shadow List Failed",
        )
        raise typer.Exit(code=1) from None
...
def shadow_status(
    ...
    try:
        ...
        shadow_error = ensure_shadow_error(
            exc,
            code="shadow_status_failed_cli",
            phase="cli_shadow_status",
            details={"shadow_id": shadow_id},
        )
        log.error("shadow_status_failed", error=shadow_error.to_dict())
        _print_shadow_error_panel(
            console,
            shadow_error,
            code="shadow_status_failed_cli",
            phase="cli_shadow_status",
            title="Shadow Status Failed",
        )
        raise typer.Exit(code=1) from None
    ...
    if env.error:
        ...
        if parsed_error:
            console.print(
                f"Error: [red]{parsed_error.code}[/red] "
                f"({parsed_error.phase}) {parsed_error.message}"
            )
        else:
            console.print(f"Error: [red]{env.error}[/red]")
    console.print()
...
def shadow_wait(
    ...
    with console.status(f"[bold blue]Waiting for {shadow_id} to be ready..."):
        try:
            ...
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_wait_failed_cli",
                phase="cli_shadow_wait",
                details={"shadow_id": shadow_id, "timeout": timeout},
            )
            log.error("shadow_wait_failed", error=shadow_error.to_dict())
            _print_shadow_error_panel(
                console,
                shadow_error,
                code="shadow_wait_failed_cli",
                phase="cli_shadow_wait",
                title="Shadow Wait Failed",
            )
            raise typer.Exit(code=1) from None
...
def shadow_delete(
    ...
    try:
        ...
        shadow_error = ensure_shadow_error(
            exc,
            code="shadow_delete_lookup_failed_cli",
            phase="cli_shadow_delete_lookup",
            details={"shadow_id": shadow_id},
        )
        log.error("shadow_delete_lookup_failed", error=shadow_error.to_dict())
        _print_shadow_error_panel(
            console,
            shadow_error,
            code="shadow_delete_lookup_failed_cli",
            phase="cli_shadow_delete_lookup",
            title="Shadow Delete Failed",
        )
        raise typer.Exit(code=1) from None
    ...
    with console.status(f"[bold yellow]Deleting shadow environment: {shadow_id}..."):
        try:
            ...
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_delete_failed_cli",
                phase="cli_shadow_delete",
                details={"shadow_id": shadow_id},
            )
            log.error("shadow_delete_failed", error=shadow_error.to_dict())
            _print_shadow_error_panel(
                console,
                shadow_error,
                code="shadow_delete_failed_cli",
...
def shadow_verify(
    ...
    with console.status(f"[bold blue]Running verification on {target_label} for {duration}s..."):
        try:
            ...
            shadow_error = ensure_shadow_error(
                e,
                code="shadow_verify_failed_cli",
                phase="cli_shadow_verify",
                details={"shadow_id": shadow_id, "duration": duration, "ephemeral": ephemeral},
            )
            log.error("shadow_verify_failed", error=shadow_error.to_dict())
            _print_shadow_error_panel(
                console,
                shadow_error,
                code="shadow_verify_failed_cli",
                phase="cli_shadow_verify",
                title="Shadow Verify Failed",
            )
            raise typer.Exit(code=1) from None

    # Refresh env to get updated status (non-fatal if lookup fails after verification).
    env = None
    if shadow_id:
        try:
            env = shadow_manager.get_environment(shadow_id)
            ...
            shadow_error = ensure_shadow_error(
                exc,
                code="shadow_verify_status_refresh_failed_cli",
                phase="cli_shadow_verify_status_refresh",
                details={"shadow_id": shadow_id},
            )
            log.warning("shadow_verify_status_refresh_failed", error=shadow_error.to_dict())
            _print_shadow_error_panel(
                console,
                shadow_error,
                code="shadow_verify_status_refresh_failed_cli",
                phase="cli_shadow_verify_status_refresh",
                title="Shadow Verify Status Refresh Failed",
            )
...
def operator_run(
    ...
    console.print(
        "RCA: "
        f"[green]{settings.agent.rca_provider.value}[/green] "
        f"({settings.agent.rca_model})"
    )
    console.print(
        "Solution: "
