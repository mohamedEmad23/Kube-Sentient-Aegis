...
def _matches_namespace(event: dict[str, Any] | str, namespace: str) -> bool:
    """Return True if event is associated with a given Kubernetes namespace."""
    if isinstance(event, dict):
        event_ns = _extract_namespace_from_event(event)
        if event_ns == namespace:
            return True
        return namespace in str(event.get("output", ""))
    return namespace in str(event)


def _priority_from_event(event: dict[str, Any] | str) -> str:
    """Extract best-effort priority from a Falco event."""
    if isinstance(event, dict):
        return _extract_priority_from_event(event)
    text = str(event).lower()
    for priority in FALCO_PRIORITY_ORDER:
        if priority.lower() in text:
            return priority
    return ""


def _summary_bucket(priority: str) -> str:
...
def _filter_alerts(
    ...
    for line in lines:
        ...
        if not parsed or not _matches_namespace(parsed, namespace):
            continue

        priority = _priority_from_event(parsed)
        if priority and not _meets_severity_threshold(priority, severity_threshold):
            continue

        summary[_summary_bucket(priority)] += 1
...
async def check_falco_alerts(
    ...
    since_minutes = max(1, min(math.ceil(delta.total_seconds() / 60), fallback_since_minutes))
    ...
    # Keep order and drop duplicates/empty values.
    seen: set[str] = set()
    ...
    log.info(
        ...
        namespace=namespace,
        falco_namespace=falco_namespace,
        label_selector=selector_candidates[0],
    ...
    try:
        ...
        stderr = b""
        returncode = 1
        used_selector = selector_candidates[0]
        ...
        for selector in selector_candidates:
            ...
            if returncode == 0:
                ...
                break
            if "no resources found" in stderr_text.lower():
                log.debug(
                    "falco_selector_retry",
                    selector=selector,
                    falco_namespace=falco_namespace,
                    stderr=stderr_text,
                )
        ...
        result["label_selector"] = used_selector
        ...
        if returncode != 0:
            result["skipped"] = True
            result["reason"] = (
                f"kubectl logs failed (exit={returncode}): {stderr_text or 'unknown error'}"
            )
            log.warning(
        ...
        if filtered_alerts:
            result["passed"] = False
            result["reason"] = (
                f"Found {len(filtered_alerts)} Falco alert(s) in namespace {namespace}"
            )
            log.warning(
...
class FalcoMonitor:
    """Facade for Falco runtime alert checks."""
    ...
    async def analyze_alerts(
        ...
        *,
        ...
        """Analyze Falco alerts for a namespace.

        Args:
            namespace: Namespace to filter alerts for (shadow namespace).
            core_api: Unused, present for interface compatibility.
            since_minutes: Lookback window in minutes.
        """
        del core_api
        since_timestamp = datetime.now(UTC) - timedelta(minutes=since_minutes)
        return await check_falco_alerts(
            namespace=namespace,
            since_timestamp=since_timestamp,
            severity_threshold=settings.security.falco_severity_threshold,
            falco_namespace=settings.security.falco_namespace,
            label_selector=settings.security.falco_label_selector,
        )


__all__ = ["FalcoMonitor", "check_falco_alerts"]
